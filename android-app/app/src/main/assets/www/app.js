...
function currentJoinPath() {
  if (nativeApp) return "/join/allow";
...
function secureNfcUrl() {
  if (nativeApp) return location.href;
...
function stopNfcOperation() {
  if (nativeApp && window.AndroidNfc) {
    window.AndroidNfc.cancel();
    if (nativeWriteResolve) {
      nativeWriteResolve({ok: false, message: "cancelled"});
      nativeWriteResolve = null;
    }
    return;
  }
  if (nfcAbort) {
...
function requireWebNfc() {
  if (nativeApp) {
    if (!window.AndroidNfc || !window.AndroidNfc.isAvailable()) throw new Error("Android NFC is not available");
    return;
  }
...
async function startPlayerNfc() {
  ...
  try {
    if (nativeApp) {
      requireWebNfc();
      window.AndroidNfc.startRead();
      playerNfcReader = true;
...
window.onNativeNfcStatus = (message) => {
  setNfcStatus(message);
  if ($("nfcAdminStatus")) $("nfcAdminStatus").textContent = message;
  if ($("nfcWriteStatus") && !nativeWriteResolve) $("nfcWriteStatus").textContent = message;
};

window.onNativeNfcRead = async (payload) => {
  const now = Date.now();
  if (!adrenalineSteal && payload === lastNfcPayload && now - lastNfcAt < 1400) return;
  lastNfcPayload = payload;
  lastNfcAt = now;
  if (!$("nfcPanel").classList.contains("hidden")) {
    const info = payloadInfo(payload);
    $("nfcAdminStatus").textContent = info.message;
    if (!adrenalineSteal) return;
  }
  try {
    await submitScanPayload(payload);
  } catch (e) {
...
window.onNativeNfcWrite = (ok, message) => {
  if (nativeWriteResolve) {
    nativeWriteResolve({ok, message});
    nativeWriteResolve = null;
  }
};
...
async function refresh() {
  ...
  if (!nativeApp && storedJoin && state.join && storedJoin !== state.join) {
    expireTab();
    return;
...
async function scanNfc() {
  if (!nativeApp && needsSecureNfcContext()) {
...
window.writeNfcItem = async (item) => {
  try {
    ...
    if (nativeApp) {
      ...
      const result = await nativeWriteTag(token.payload);
      if (!result.ok) throw new Error(result.message || "NFC write failed");
